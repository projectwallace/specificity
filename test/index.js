...
describe('CALCULATE_FOR_SELECTOR_AST', () => {
    describe('Examples from the spec', () => {
        it('* = (0,0,0)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('*')).toObject(), { a: 0, b: 0, c: 0 });
        });
        it('li = (0,0,1)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('li')).toObject(), { a: 0, b: 0, c: 1 });
        });
        it('ul li = (0,0,2)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('ul li')).toObject(), { a: 0, b: 0, c: 2 });
        });
        it('UL OL+LI  = (0,0,3)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('UL OL+LI ')).toObject(), { a: 0, b: 0, c: 3 });
        });
        it('H1 + *[REL=up] = (0,1,1)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('H1 + *[REL=up]')).toObject(), { a: 0, b: 1, c: 1 });
        });
        it('UL OL LI.red = (0,1,3)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('UL OL LI.red')).toObject(), { a: 0, b: 1, c: 3 });
        });
        it('LI.red.level = (0,2,1)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('LI.red.level')).toObject(), { a: 0, b: 2, c: 1 });
        });
        it('#x34y = (1,0,0)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('#x34y')).toObject(), { a: 1, b: 0, c: 0 });
        });
        it('#s12:not(FOO) = (1,0,1)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('#s12:not(FOO)')).toObject(), { a: 1, b: 0, c: 1 });
        });
        it('.foo :is(.bar, #baz) = (1,1,0)', () => {
            deepEqual(Specificity.calculateForAST(parse_selector('.foo :is(.bar, #baz)')).toObject(), { a: 1, b: 1, c: 0 });
        });
    });
});
