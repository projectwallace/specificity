...
const Calculate = suite('Calculate')

Calculate('calculates specificity', () => {
  const fixtures = [
  ...
  fixtures.forEach(({ selector, expected }) => {
    const actual = calculate(selector)
    assert.equal(actual.specificity, expected)
  })
})

Calculate(`explains parts of the specificity`, () => {
  const fixtures = [
    {
      selector: 'test',
      expected: [
        { selector: 'test', specificity: [0, 0, 1] }
      ]
    },
    {
      selector: '.qux:where(em, #foo#bar#baz)',
      expected: [
        { selector: '.qux', specificity: [0, 1, 0] },
        { selector: ':where(em, #foo#bar#baz)', specificity: [0, 0, 0] },
      ]
    },
    {
      selector: 'a:hover:is(h1, h2 img, h3 + .foo.bar)',
      expected: [
        { selector: 'a', specificity: [0, 0, 1] },
        { selector: ':hover', specificity: [0, 1, 0] },
        { selector: ':is(h1, h2 img, h3 + .foo.bar)', specificity: [0, 2, 1] },
      ]
    }
  ]
  ...
  fixtures.forEach(({ selector, expected }) => {
    const actual = calculate(selector)
    assert.equal(actual.parts, expected)
  })
})

Calculate('returns the original selector', () => {
  const fixtures = [
    'selector',
    'test:where(#a, .b)',
    'li:nth-chuld(3n + 4) > x'
  ...
  fixtures.forEach(fixture => {
    ...
    assert.is(actual.selector, fixture)
  })
})
